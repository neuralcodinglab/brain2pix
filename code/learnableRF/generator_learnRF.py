...
class Generator:
    ...
    def train(self, d: HybridSequential, to_learn_x: List[NDArray], brain_RF:NDArray, y: NDArray) -> float:
        ...
        with autograd.record():
            if self._train_RF:
                learned_RF = self._rf_mapper(to_learn_x)
                merged_RF = learned_RF + brain_RF 
            loss = (lambda y_hat: self.lossfun(1, d(concat(merged_RF, y_hat, dim=1)), y, y_hat))(self._network(merged_RF))
