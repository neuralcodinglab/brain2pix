...
def make_iterator_preprocessed(set_t, *roi, shift=2, time_range=5, batch_size=16, shuffle=False, synthetic=False):
    
    targets = np.load(f'{rel_dir}vim2_pre_processed_data/{set_t}/targets_warped.npy').transpose((0,3,1,2)).astype('float32')
    signals_per_roi = []
    if synthetic:
        for roi_i in roi:
            signals = np.load(f'{rel_dir}vim2_pre_processed_data/{set_t}/synthetic-{roi_i}.npy').astype('float32')
            signals_per_roi.append(signals)
    else:
        for roi_i in roi:
            signals = np.load(f'{rel_dir}vim2_pre_processed_data/{set_t}/{roi_i}-shift={shift},time_range{time_range}.npy').astype('float32')
            signals_per_roi.append(signals)

    difference = -(signals_per_roi[1].shape[0] - targets.shape[0])
